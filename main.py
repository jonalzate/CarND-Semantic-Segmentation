...
def layers(vgg_layer3_out, vgg_layer4_out, vgg_layer7_out, num_classes):
    ...
    # Upsample Layer
    layer4_upsample = tf.layers.conv2d_transpose(layer7_conv1x1, num_classes, 4, 
                                             strides= (2, 2), 
                                             padding= 'same', 
                                             kernel_initializer= tf.random_normal_initializer(stddev=0.01), 
                                             kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    ...
    layer4_out = tf.add(layer4_upsample, layer4_conv1x1)
    
    # upsample layer
    layer3_upsample = tf.layers.conv2d_transpose(layer4_out, num_classes, 4,  
                                             strides= (2, 2), 
                                             padding= 'same', 
                                             kernel_initializer= tf.random_normal_initializer(stddev=0.01), 
                                             kernel_regularizer= tf.contrib.layers.l2_regularizer(1e-3))
    ...
    # Skip layer connection
    layer3_out = tf.add(layer3_upsample, layer3_conv1x1)
...
def optimize(nn_last_layer, correct_label, learning_rate, num_classes):
    ...
    # Logits a 2D tensor where each row represents a pixel and each column a class
...
def train_nn(sess, epochs, batch_size, get_batches_fn, train_op, cross_entropy_loss, input_image,
    ...
    """
    # Initialiaze global variables and start training session
    # Iterate over batches for image and label then pass them to the model
    ...
    for i in range(epochs):
        ...
        for image, label in get_batches_fn(batch_size):
            op, loss = sess.run([train_op, cross_entropy_loss], 
